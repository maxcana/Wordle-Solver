...

{
    ...
    static WordleSolver()
    {
        ...
        cachedRankings = new Dictionary<string, float>(){
    ...
    private void Awake() 
    {
        ...
        wordRankings = new Dictionary<string, float>();
    ...
    public static bool calcWordRankings(IDictionary<char, CharKnowledge> knowledge)
    {
        ...
        try
        {
            SortedList<(float, string), bool> currentWordRankings = new SortedList<(float, string), bool>(new PairComparer());
    ...
    public static List<string> BitFilter(ulong[] mask, ulong[] possibleSecretsBitmap, List<string> possibleSecrets)
    {
    ...
    public enum Colour { Green, Yellow, Gray, Missing }
    ...
    public class CharKnowledge
    {
        ...
        {
            return ""
            + " definite: " + "{" + String.Join(",", definitePositions) + "}"
            + " unavail: " + "{" + String.Join(",", unavailablePositions) + "}"
            + " grayBeyond: " + (grayBeyond == null ? "null" : grayBeyond + "");
        }

        static IDictionary<char, ISet<int>> stringToCharPositions(string s, IDictionary<char, ISet<int>> constrain)
        {
        ...
        public static List<Colour> getColoursForWordle(string guess, string actualWord)
        {
            ...
            foreach (char c in guess)
            {
                if (!counts.ContainsKey(c))
                    counts.Add(c, 0);
                result.Add(map.ContainsKey(c)
                               ? map[c].Contains(pos)
                                   ? Colour.Green
                                   : counts[c]++ < (map[c].Count - hits[c].Count) ? Colour.Yellow : Colour.Gray
                               : Colour.Gray);
                pos++;
            }
            return result;
        ...
        public static List<Colour> getColoursForWord(string guess, string actualWord)
        {
            ...
            for (int i = 0; i < guessList.Count; i++)
            {
                ...
                bool guessedNotMore = getAmountOfInListThatAreNotGray(guessList[i], guessList, colours) <= getAmountOfInList(guessList[i], actualWordList);
            ...
            {
                int amountOfNonGraysInGuess = getAmountOfInListThatAreNotGray(guessList[i], guessList, colours); //2
        ...
        public static int getAmountOfInListThatAreNotGray(char charToCheck, List<char> list, List<Colour> colours)
        {
            int toReturn = 0;
        ...
        public CharKnowledge Combine(CharKnowledge that)
        {
            ...
            if (this.grayBeyond != that.grayBeyond && this.grayBeyond != null && that.grayBeyond != null)
                throw new Exception("kaboom");

            int thisHasAtLeastThisManyInTotal = this.atLeastThisManyMore + this.definitePositions.Count;
            int thatHasAtLeastThisManyInTotal = that.atLeastThisManyMore + that.definitePositions.Count;
            int resultHasAtLeastThisManyInTotal = Math.Max(thisHasAtLeastThisManyInTotal, thatHasAtLeastThisManyInTotal);
            result.atLeastThisManyMore = Math.Max(resultHasAtLeastThisManyInTotal - result.definitePositions.Count, 0);
        ...
        public static ISet<int> calcUnsolvedPositions(int wordLength, ICollection<CharKnowledge> values)
        {
            var result = new HashSet<int>();
            for (int i = 0; i < wordLength; i++)
                result.Add(i);
            foreach (var value in values)
                foreach (var position in value.definitePositions)
        ...
        public static IDictionary<char, CharKnowledge> infer(int wordLength, IDictionary<char, CharKnowledge> input)
        {
            ...
            foreach (KeyValuePair<char, CharKnowledge> kv in input)
            {
                ...
                if (availableUnsolvedPositions.Count == val.atLeastThisManyMore && availableUnsolvedPositions.Count != 0)
                {
            ...
            if (slack == 0)
            {
                ...
                for (char c = 'a'; c <= 'z'; c++)
                {
                    if (input.ContainsKey(c))
                    {
                        ...
                        {
                            modified = true;
                            result[c].grayBeyond = result[c].definitePositions.Count + result[c].atLeastThisManyMore;
                        }
                    }
                    else
                    {
        ...
        public static T fixpoint<T>(Func<T, T> f, T input) where T : class
        {
            T old;
            do
            {
        ...
        public static IDictionary<char, CharKnowledge> inferFully(int wordLength, IDictionary<char, CharKnowledge> input)
        {
            Func<IDictionary<char, CharKnowledge>, IDictionary<char, CharKnowledge>> infer = (x) => { return CharKnowledge.infer(wordLength, x); };
        ...
        private static IDictionary<char, CharKnowledge> copyDictionary(IDictionary<char, CharKnowledge> source)
        {
            var result = new Dictionary<char, CharKnowledge>();
            foreach (var kv in source)
                result.Add(kv.Key, new CharKnowledge(kv.Value));
            return result;
        }
        public static ISet<ISet<T>> ItemCombinations<T>(int k, ISet<T> set)
        {
            ...
            {
                var s = new HashSet<T>();
                foreach (int index in indexComb)
                    s.Add(order[index]);
                result.Add(s);
            }
            return result;
        ...
        public static IDictionary<char, CharKnowledge> fromGuess(string guess, List<Colour> guessResult)
        {
            ...
            foreach (KeyValuePair<char, ISet<KeyValuePair<int, Colour>>> kv in charInfo)
            {
                ...
                foreach (var pair in kv.Value)
                {
                    ...
                    {
                        charKnowledge.unavailablePositions.Add(pair.Key);
                        charKnowledge.grayBeyond = charKnowledge.atLeastThisManyMore + charKnowledge.definitePositions.Count;
                    }
                }
