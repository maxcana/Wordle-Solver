...

{
    ...
    public static long currentIteration;
    public static long iterationCount;
    ...
    {
        currentIteration = 0L;
        iterationCount = dictionary.Count * dictionary.Count * dictionary.Count;
        currentWordsTried = 0;
        currentWordsTriedDecimal = 0;
        currentWordsTriedPerSecond = 0;
    }
    ...
    {
        iterationCount = (long)dictionary.Count * (long)possibleSecrets.Count * (long)possibleSecrets.Count;
        currentIteration = 0L;
        currentWordsTried = 0;
        ...
        try
        {
            ...
            foreach (var word in dictionary)
            {
                ...
                foreach (var possiblesecret in possibleSecrets)
                {
                    ...
                    // string pattern = CharKnowledge.pattern(CharKnowledge.startMaxDepth, WordleData.wordLength, newKnowledge);
                    // var re = new Regex("^(" + pattern + ")$");
                    // int xprime = pattern == null ? 0 : filter(possibleSecrets, new Regex("^(" + pattern + ")$"), true).Count;
                    ...
                    totalxprime += xprime;
                    float oldCurrentWordsTriedDecimal = currentWordsTriedDecimal;
                    currentWordsTriedDecimal += 1 / possibleSecrets.Count();
                    currentWordsTriedPerSecond = (currentWordsTriedDecimal - oldCurrentWordsTriedDecimal) / Time.time - time;
                    //! THIS DOESN't WORK (TIME CAN ONLY BE CALLED FROM MAIN THREAD@!)
                }
